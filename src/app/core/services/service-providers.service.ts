...
export class ServiceProvidersService {
    ...
    async generateServiceProviderSecurityToken(networkId: number,request: {security_authentication_type: string;bearer_token_expiration_seconds?: number | null;}): Promise<ResponseI> {
        return this.connectionService.send(
            `service-provider/${networkId}/security-token/generate`,
            'post',
            request
        );
    }
