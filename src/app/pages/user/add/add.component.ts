...
export class AddComponent implements OnInit {
  ...
  showSenderIdWarning: boolean = false;
  ...
  async save(): Promise<void> {
    ...
    const roles: string[] = this.form.getRawValue().roles || [];
    const isCampaignRole = roles.includes('CAMPAIGN_OPERATOR') || roles.includes('CAMPAIGN_APPROVER');
    const senderIds = this.form.getRawValue().sender_ids || [];
    if (isCampaignRole && senderIds.length === 0) {
      this.showSenderIdWarning = true;
      return;
    }
  ...
  private async proceedSave(): Promise<void> {
  ...
  onSenderIdWarningAccept(): void {
    this.showSenderIdWarning = false;
    this.btnEnabled = false;
    this.proceedSave();
  }

  onSenderIdWarningCancel(): void {
    this.showSenderIdWarning = false;
  }
