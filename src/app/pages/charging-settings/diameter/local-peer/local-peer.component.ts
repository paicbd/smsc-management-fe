...
const IPV4_REGEX = /^((25[0-5]|2[0-4]\d|[01]?\d\d?)\.(25[0-5]|2[0-4]\d|[01]?\d\d?)\.(25[0-5]|2[0-4]\d|[01]?\d\d?)\.(25[0-5]|2[0-4]\d|[01]?\d\d?))(\s*,\s*(25[0-5]|2[0-4]\d|[01]?\d\d?)\.(25[0-5]|2[0-4]\d|[01]?\d\d?)\.(25[0-5]|2[0-4]\d|[01]?\d\d?)\.(25[0-5]|2[0-4]\d|[01]?\d\d?))*$/;
...
export class LocalPeerComponent implements OnInit, OnDestroy {
    ...
    initializeForm(): void {
        this.localPeerForm = this.fb.group({
            ...
            uri: ['', [Validators.required, Validators.pattern(/^[a-zA-Z][a-zA-Z0-9+.-]*:\/\/[a-zA-Z0-9._-]+(:[0-9]+)?$/)]],
            realm: ['', [Validators.required]],
            ip_addresses: ['', [Validators.required, Validators.minLength(1), Validators.maxLength(environment.MaxLengthIp), Validators.pattern(IPV4_REGEX)]],
    ...
    emitChanges(source: 'form' | 'applications' = 'form') {
        ...
        this.localPeerChange.emit({
            localPeer: { ...this.localPeer, ...this.localPeerForm.getRawValue() },
            applications: this.applicationList,
            modified,
            source,
        });
    ...
    markFormAsTouched(): void {
        this.localPeerForm.markAllAsTouched();
    }

    getInvalidFieldLabels(): string[] {
        const labelMap: Record<string, string> = {
            uri: 'Local Peer - URI',
            realm: 'Local Peer - Realm',
            ip_addresses: 'Local Peer - IP Addresses',
            vendor_id: 'Local Peer - Vendor ID',
            product_name: 'Local Peer - Product Name',
            firmware_version: 'Local Peer - Firmware Version',
        };
        return Object.entries(labelMap)
            .filter(([key]) => { const ctrl = this.localPeerForm.get(key); return ctrl?.invalid && ctrl?.enabled; })
            .map(([, label]) => label);
    }
    ...
    getPatternMessage(name: string) {
        ...
        } else if (name === 'ip_addresses' || name === 'ip') {
            return 'Invalid IP address format';
        } else if (this.localPeerForm.get(name)?.errors?.['pattern']?.requiredPattern == '^[A-Za-z0-9]*$') {
