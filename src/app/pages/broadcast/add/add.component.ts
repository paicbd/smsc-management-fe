...
export class AddComponent implements OnInit, OnDestroy {
  ...
  messageEventFieldsForSelect: any[] = [];
  private allMessageEventFields: any[] = [];
  private allMessageEventFieldsGrouped: any[] = [];
  ...
  async loadInitData() {
    ...
    this.isAdminOrRoot = roles.includes('ROOT') || roles.includes('ADMINISTRATOR');
  ...
  toggleAdvancedSettings(): void {
    ...
    if (!this.showAdvancedSettings) {
      ...
      const advancedDefaults: { [key: string]: any } = {
        ...
        dest_addr_ton: 1, dest_addr_npi: 1, data_coding: environment.broadcastDefaultDcs ?? 0
      };
  ...
  private updateFilteredFields(): void {
    if (this.showAdvancedSettings) {
      ...
      this.messageEventFieldsForSelect = this.allMessageEventFieldsGrouped;
    } else {
      this.messageEventFieldsGrouped = this.allMessageEventFields.filter(f => f.group === 'Basic');
      this.messageEventFieldsForSelect = this.allMessageEventFieldsGrouped.filter((g: any) => g.group === 'Basic');
    }
  }
  ...
  private getInitialBroadcast(): Broadcast {
    return {
      ...
      dest_addr_ton: 0,
      dest_addr_npi: 0,
      data_coding: environment.broadcastDefaultDcs ?? 0,
      is_immediate: true,
    };
  }
  ...
  async loadMessageEventFields() {
    ...
    if (response.status === 200 && Array.isArray(response.data)) {
      this.allMessageEventFieldsGrouped = response.data;
  ...
  onMappingChange(event: any, column: string, position: number): void {
    const control = this.form.get(column);
    if (!control) return;

    const selectedKey = event?.target ? event.target.value : (event?.key || '');

    if (selectedKey) {
      const alreadyUsed = Object.entries(this.columnMapping).some(
        ([key, val]) => val === selectedKey && key !== column
      );

      if (alreadyUsed) {
        this.alertSvr.showAlert(2, 'Mapping Conflict', `"${selectedKey}" has already been assigned to another column.`);
        control.setValue('');
        this.columnMapping[column] = '';
        return;
      }

      this.columnMapping[column] = selectedKey;
    } else {
      this.columnMapping[column] = '';
    }
  ...
  clearMapping(column: string): void {
    const control = this.form.get(column);
    if (control) {
      control.setValue('');
      this.columnMapping[column] = '';
  ...
  private isFormValid(): boolean {
    ...
    if (!senderIdControl?.value || String(senderIdControl.value).trim() === '') {
      this.alertSvr.showAlert(2, 'Sender Required', 'Please provide a Sender ID.');
      return false;
    }
