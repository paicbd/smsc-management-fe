...
interface ImportMetaEnv {
  ...
  readonly NG_APP_BROADCAST_DEFAULT_DCS: number;
  //ip-sm-gw
  readonly NG_APP_ENABLE_IP_SM_GW: boolean;
  readonly NG_APP_PREMIUM_REDIRECT: string;

  // # Variables for SIP Settings Defaults
  readonly NG_APP_SIP_STATUS: string;
  readonly NG_APP_SIP_PROTOCOL: string;

  readonly NG_APP_SIP_IP: string;
  readonly NG_APP_SIP_NAME: string;
  readonly NG_APP_SIP_PORT: number;
  readonly NG_APP_SIP_TRANSPORT: string;
  readonly NG_APP_SIP_MESSAGES_PER_SECOND: number;
  readonly NG_APP_SIP_MESSAGES_PER_SECOND_HIGH: number;
  readonly NG_APP_SIP_MESSAGES_PER_SECOND_MEDIUM: number;
  readonly NG_APP_SIP_MESSAGES_PER_SECOND_LOW: number;

  readonly NG_APP_SIP_TRANSACTION_TIMEOUT: number;
  readonly NG_APP_SIP_RETRANSMISSION_BASE_INTERVAL_MS: number;
  readonly NG_APP_SIP_RETRANSMISSION_MAX_INTERVAL_MS: number;
  readonly NG_APP_SIP_NETWORK_TIMEOUT_MS: number;

  readonly NG_APP_SIP_THREAD_POOL_SIZE: number;
  readonly NG_APP_SIP_RETRANSMISSION_FILTER: boolean;
  readonly NG_APP_SIP_MAX_MESSAGE_SIZE: number;
  readonly NG_APP_SIP_SPLIT_MESSAGE: boolean

  readonly NG_APP_SIP_RECEIVE_UDP_BUFFER_SIZE: number;
  readonly NG_APP_SIP_SEND_UDP_BUFFER_SIZE: number;
  readonly NG_APP_SIP_AGGRESSIVE_CLEANUP: boolean;

  readonly NG_APP_SIP_ROUTING_ENABLE_SS7: boolean;
  readonly NG_APP_SIP_ROUTING_ENABLE_DIAMETER: boolean;

  readonly NG_APP_SIP_ENABLED: number;

  readonly NG_APP_SIP_REGISTER_MAX_EXPIRES: number;

  readonly NG_APP_SIP_IPSMGW_USER: string;
  readonly NG_APP_SIP_IPSMGW_DOMAIN: string;

  readonly NG_APP_SIP_IMS_DOMAIN: string;
  readonly NG_APP_SIP_IMS_CCF: string;
  readonly NG_APP_SIP_IMS_ECF: string;

  readonly NG_APP_SIP_SUBSCRIBE_TARGET_HOST: string;
  readonly NG_APP_SIP_SUBSCRIBE_TARGET_PORT: number;
  readonly NG_APP_SIP_SUBSCRIBE_TARGET_TRANSPORT: string;

  readonly NG_APP_SIP_LOCAL_VIA_HOST: string;
  readonly NG_APP_SIP_GLOBAL_TITLE: number;
  readonly NG_APP_SIP_USSI_DEFAULT_DATACODING_ID: number;
